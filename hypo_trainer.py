...
from hypo_config import DPOConfig, FDivergenceConstants, FDivergenceType
...
class DPOTrainer(Trainer):
    ...
    def dpo_loss(
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE.value:
            ...
            if getattr(self.args, "im_enable", True):
                gamma = torch.as_tensor(self.args.im_gamma, dtype=ref_logratios.dtype, device=ref_logratios.device)
                if getattr(self.args, "im_tau", 0.0) and self.args.im_tau > 0.0:
                    tau = float(self.args.im_tau)
        ...
        if getattr(self.args, "im_enable", True):
            ref_c = reference_chosen_logps.to(self.accelerator.device)
            ref_r = reference_rejected_logps.to(self.accelerator.device)
            delta_ref = ref_c - ref_r
            gamma = torch.as_tensor(self.args.im_gamma, dtype=delta_ref.dtype, device=delta_ref.device)

            if getattr(self.args, "im_tau", 0.0) and self.args.im_tau > 0.0:
                tau = float(self.args.im_tau)
                delta_ref_prime = gamma + tau * F.softplus((delta_ref - gamma) / tau)
            else:
                delta_ref_prime = torch.maximum(delta_ref, gamma)

            gap = torch.clamp(delta_ref_prime - delta_ref, min=0.0)
            ref_c_prime = ref_c + 0.5 * gap
            ref_r_prime = ref_r - 0.5 * gap

            chosen_rewards = (self.beta * (policy_chosen_logps - ref_c_prime)).detach()
            rejected_rewards = (self.beta * (policy_rejected_logps - ref_r_prime)).detach()
        else:
            chosen_rewards = self.beta * (
                    policy_chosen_logps.to(self.accelerator.device) - reference_chosen_logps.to(self.accelerator.device)
            ).detach()
            rejected_rewards = self.beta * (
                    policy_rejected_logps.to(self.accelerator.device) - reference_rejected_logps.to(
    ...
    def get_batch_logps(
            ...
            exclude_eos_from_average: bool = False,
        ...
        if average_log_prob and exclude_eos_from_average and eos_token_id is not None:

            lengths = loss_mask.sum(dim=-1)  # [B]
            has_tokens = lengths > 0
            last_idx = (lengths - 1).clamp(min=0)
            bsz = labels.size(0)
            arange = torch.arange(bsz, device=labels.device)

            last_labels = labels[arange, last_idx]
            need_drop = has_tokens & (last_labels == eos_token_id)
            if need_drop.any():
                loss_mask[arange[need_drop], last_idx[need_drop]] = False


        safe_labels = labels.clone()
        safe_labels[safe_labels == label_pad_token_id] = 0
        per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=safe_labels.unsqueeze(2)).squeeze(2)

        token_sums = (per_token_logps * loss_mask).sum(-1)
        lengths = loss_mask.sum(-1)

        if average_log_prob:
            token_sums = token_sums / lengths.clamp_min(1)

        return token_sums, lengths
    ...
    def concatenated_forward(
        ...
        avg_flag = getattr(self.args, "use_average_log_prob", getattr(self.args, "avg_log_prob", False))

        all_logps, size_completion = self.get_batch_logps(
            all_logits,
            concatenated_batch["concatenated_labels"],
            average_log_prob=avg_flag,  
            is_encoder_decoder=self.is_encoder_decoder,
            label_pad_token_id=self.label_pad_token_id,
            eos_token_id=(self.tokenizer.eos_token_id if hasattr(self, "tokenizer") else None),
            exclude_eos_from_average=True,  
        )
    ...
    def get_batch_loss_metrics(
        ...
        # Use precomputed reference log-probs if available; otherwise run the reference model.
        ...
        with torch.no_grad():
            delta_theta = policy_chosen_logps - policy_rejected_logps
            if self.reference_free:
                delta_ref = torch.zeros_like(delta_theta)
            else:
                delta_ref = reference_chosen_logps - reference_rejected_logps

            if getattr(self.args, "im_enable", True):
                gamma = torch.as_tensor(
                    getattr(self.args, "im_gamma", 0.0),
                    dtype=delta_ref.dtype,
                    device=delta_ref.device,
                )
                if getattr(self.args, "im_tau", 0.0) and self.args.im_tau > 0.0:
                    tau = float(self.args.im_tau)
                    delta_ref_prime = gamma + tau * F.softplus((delta_ref - gamma) / tau)
                ...
                delta_ref_prime = delta_ref

            eff_margin = delta_theta - delta_ref_prime
        ...
        metrics[f"{prefix}rewards/margins"] = (self.beta * eff_margin).mean().detach().cpu()
